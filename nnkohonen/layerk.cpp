...
void Kohonen_network::get_layer_info(int inputlayersize, int outputlayersize)
{
	//------------------------------------------
	//
	// 	Set layer sizes for the Kohonen network 
	//	
	// -----------------------------------------                                            
	layer_size[0]=inputlayersize;
	layer_size[1]=outputlayersize;
	// ------------------------------------------------------
	// size of layers: 
...
void Kohonen_network::set_up_network(int nsz)
{
	...
	layer_ptr[1]=new Kohonen_layer(layer_size[0], layer_size[1], neighborhood_size);
...
{
	((Kohonen_layer *)layer_ptr[1])->randomize_weights();
}

void Kohonen_network::update_neigh_size(int n)
{
	((Kohonen_layer *)layer_ptr[1])->update_neigh_size(n);
}

void Kohonen_network::update_weights(const float a)
{
	((Kohonen_layer *)layer_ptr[1])->update_weights(a);
}

void Kohonen_network::list_weights()
{
	((Kohonen_layer *)layer_ptr[1])->list_weights();
}

void Kohonen_network::list_outputs()
{
	((Kohonen_layer *)layer_ptr[1])->list_outputs();
}

void Kohonen_network::get_next_vector(float *inputvector)
{
...
{
	return ((Kohonen_layer *)layer_ptr[1])->get_win_dist();
}
